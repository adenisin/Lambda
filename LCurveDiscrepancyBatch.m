function [lambda,lambda_Discrepancy,files] = LCurveDiscrepancyBatch(E,conversion)
nu = 0.49;
A=(1+nu)/(pi*E);
...
lambda = zeros(size(files,1),1);
%reg_param = zeros(size(files,1),1);
...
for z = 1:size(files,1)
    ...
    rho_abs(:,:,z) = rho(:,:,z)/((conversion*1e-6)^2);
    rho_c_abs = rho_c/((conversion*1e-6)^2);
    
%     figure(3)
%     figure3 = loglog(reg_param(:,z), rho_abs);
%     xlabel('Lambda');
%     ylabel('residual norm || A x - b ||_2');
%     hold on;
...
%     %# make all text in the figure to size 20
%     set(findall(figureHandle,'type','text'),'fontSize',15)
%     figure3 = plot(reg_param(:,z),rho_opt*ones(size(reg_param(:,z))), 'r');
%     figure3 = plot(reg_param(:,z),rho_opt_plus*ones(size(reg_param(:,z))), 'r:');
%     figure3 = plot(reg_param(:,z),rho_opt_minus*ones(size(reg_param(:,z))), 'r:');
%     
    % %find the lambda at the rho_opt
    val(:,z) = abs(rho_abs(:,:,z)-rho_opt);
    [idx idx] = min(val(:,z)); %index of closest value
    lambda_Discrepancy(z) = reg_param(idx,z);
